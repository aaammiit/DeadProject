...
def speechType(request,typ):
    ...
    try:
        ...
        for item in filedata:
            try:
                if (
                    ...
                    and item.KRIMA_type in ['Speeches']
                ):
        ...
        speech_data = sorted(speech_data, key=lambda x: x.Date, reverse=True)
        Frontdata = speech_data[:10]  # First 10 entries for the front page
        rangedate = speech_data[11:15]  # First 12 entries for the date range
...
def speechfilterData(request):
    try:
        ...
        if request.method == 'POST':
            ...
            return render(request, 'speech/speechFilter.html', {
...
@login_required
def findSpeechCountry(request,cntry):
    findcountry=cntry
    filetrData, rangedate, country, regulatory, cuntryLen, regLen,rbData = fetch_and_process_data()
    ...
    return render(request, 'speech/speechCountryRagPage.html', {
...
@login_required
def speechrbProfile(request, id):
    rbid = str(id)
    data = []
    AGrowth = None
    cGpd = None
 ...
 # Redirect back or to home
    ...
    return render(request, 'speech/speechRb.html', {
        'data': data,
        'logo': logo_path,
        'flag': flag_path,
        'gdp_data': gdp_data,
